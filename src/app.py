...
# ---------------Start Dashboard
st.set_page_config(
    layout="wide", page_title="Healthy City", page_icon="assets/logo.png"
)
...
data_display = st.sidebar.multiselect(
    "Directly select datasets to display:", [d.name for d in objs_filtered]
)
...
if data_display != []:
    ...
    with my_expander:
        st.markdown(
            "**Welcome to the dashboard for the Healthy Brabantine City Deal Challenge!**"
        )
        st.text("")
        st.markdown(
            "This dashboard will guide you through the several datasets that are collected for you to use in your projects.\
